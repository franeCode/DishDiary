...
const RecipeCard = ({ recipe, showIcon, customRecipeId, deleteRecipe, isEditing, setIsEditing }) => {
    ...
    <div className="col-sm-6 col-md-4 col-lg-3 mt-5">
      <div className="card align-items-center rounded-4 shadow-sm mb-5 position-relative" style={{ width: "18rem", height: "25rem", borderColor: "transparent" }}>
        ...
        <div className="card-body pt-0">
          <h5 className="card-title fs-4">{recipe.title || "No title available"}</h5>
          ...
          <p className="d-flex justify-content-start align-items-center gap-4 card-text">
            <button className="btn bg-transparent rounded" type='button' onClick={() => handleReadMore()}><i className="fa-solid fa-info fa-lg" style={{color: "#FF7D04"}}></i></button>
            {showIcon && !isEditing && (
            ...
            {showIcon && !isEditing && (
              <button className='btn text-white rounded' type="submit" onClick={() => handleDeleteClick(recipe.id)}>
                <Link className='text-decoration-none text-white' to="/custom_recipes">
